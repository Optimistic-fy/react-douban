...
class Recommond extends React.PureComponent{
    ...
    handlePraise = (id, praise) => {
        if (praise) {
            this.props.cancelPraise(id)
        } else {
                            ...
                            <li key={item.snowflakeId}>
                                <div className="recommond">
                                    ...
                                    <div className="bottom">
                                        <span className={`iconfont ${item.praise || isPraise ? 'like' : 'dislike'}`} onClick={() => this.handlePraise(item.snowflakeId, item.praise)}>&#xe61b;<span className="like-count">{item.praiseCount}</span></span>  
                                        <Link to={`/infoDetail/${item.snowflakeId}`}>
