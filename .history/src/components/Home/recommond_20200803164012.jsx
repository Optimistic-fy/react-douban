...
import { getReList, clickPraise } from './store/actionCreaters'
...
@connect(
    state => state,
    {getReList, clickPraise, changeLoading}
)
...
class Recommond extends React.PureComponent{
    ...
    onDownRefresh = async () => {
        ...
        await this.props.getReList(params)
    ...
    handlePraise = (id, praise, index) => {
        console.log('recomList', id, praise, index)
        if (praise) {
            this.props.cancelPraise(id, index)
        } else {
            this.props.clickPraise(id, index)
        }
    }
    render () {
        const { recomList, isPraise } = this.props.Home
                            ...
                            <li key={item.snowflakeId}>
                                <div className="recommond">
                                    ...
                                    <div className="bottom">
                                        <span className={`iconfont ${item.praise ? 'like' : 'dislike'}`} onClick={() => this.handlePraise(item.snowflakeId, item.praise, index)}>&#xe61b;<span className="like-count">{item.praiseCount}</span></span>  
                                        <Link to={`/infoDetail/${item.snowflakeId}`}>
